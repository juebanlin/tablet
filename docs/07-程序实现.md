...
│   │   └── .tbl-cache/         ← 临时 xlsx + .tbl.tmp，gitignore（@03、@06）
...
└── crates/
    ...
    │   └── schemas/            ← 内置 .tblschema（include_str! 嵌入；@02 项目模板 / @09 测试驱动共用）
    ├── app-cli/                ← CLI 入口
    ├── app-egui/               ← egui 桌面端（主推）
    ├── app-fltk/               ← FLTK（待开发，@05）
    └── app-slint/              ← Slint（待开发，@05）
```

`core` 不依赖任何 UI 框架，所有 UI 实现共享同一份模型与验证逻辑。`core/src/template/` 模块同时被项目模板库（@02）和 generate-test CLI（@02 测试数据生成）复用。

## 4. tbl-tool.toml 配置

工具配置文件，定义启动展开偏好、导出目标、UI 行为、类型分隔符。所有字段都有默认值，新建项目可只写少量字段。**`tbl-tool.toml` 是仓库全局配置，所有 Project 共享；Project 自身的元数据 + 可选配置覆盖放在 `projects/<id>/project.toml`（@02 Project）**。Project 的同名段（`[export]` / `[ui]` / `[separators]`）按 field-level deep-merge 覆盖全局；缺失字段回退到全局。
...
每个语言的具体路径派生规则见 @02（核心功能）。

## 6. 内部数据模型

工具运行时的内存结构，定义在 `crates/core/src/model.rs`。

`ProjectEngine` 是核心服务：启动时一次性加载 `<workdir>/projects/` 下**全部** Project 到内存（`Vec<Project>`），不再持有"当前 project"概念。每个 `Project` 自带独立的 `project_root` / `instance_meta` / `groups`，所有节点级操作（find_table / validate / save）按 `(project_id, group, name)` 三元组定位。

```rust
pub struct ProjectEngine {
    pub workdir: PathBuf,            // 仓库根（projects/ 的父目录）
    pub config: ProjectConfig,       // 解析自 <workdir>/tbl-tool.toml；project 自身的 overlay 已 deep-merge 进各 Project.config
    pub projects: Vec<Project>,      // 全部加载的 Project，顺序 = project id 字典序
    pub validation_errors: HashSet<(String, String, String, usize, usize)>, // (project_id, group, name, row, col)
    // ... 其余字段
}

pub struct Project {
    pub project_root: PathBuf,       // projects/<id>/
    pub instance_meta: ProjectInstanceMeta,  // 解析自 project.toml 的 [project] 段
    pub config: ProjectConfig,       // 全局 + project.toml overlay 的合并结果
