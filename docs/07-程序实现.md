...
加新分隔符时，需要同步：(1) `SeparatorsSection` + serde rename；(2) `sep_kv_pairs` 顺序数组；(3) `paradigm_sep_keys` 映射表；(4) Go / Java 模板里的 SepConfig 字段及对应 fromMap / fromXMLAttrs 加载；(5) `tblschema.rs::apply_sep_kv` 的 25-case 表 + `write_sep_diff`；(6) 项目设置对话框 `project_settings.slint` 的 sep-\* property 与 SepRow。`sep_meta.rs` 的 4 个 unit test 锁死 (1)(2)(3) 一致性 —— 漏改会立刻失败。
