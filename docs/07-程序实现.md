...
└── crates/
    ...
    ├── app-cli/                ← CLI 入口（产物 tbl-cli）
    └── app-slint/              ← Slint GUI（产物 tbl-slint，@05）
```

`core` 不依赖任何 UI 框架，所有前端共享同一份模型与验证逻辑。`core/src/template/` 模块同时被项目模板库（@02）和 generate-test CLI（@02 测试数据生成）复用。
...
slint 的声明式布局有几处与命令式 UI 行为不同的地方，踩过的坑统一记在这里。
