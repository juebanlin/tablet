...
UI 设计和布局见 [@04 UI 设计](04-UI设计.md)，与具体实现平台无关。
...
- 4 大区域声明顺序：`SidePanel` / `TopBottomPanel` 先声明的先占外层空间。`LogPanel` 要"跨全宽"必须**早于** `SidePanel::left` 声明，否则 SidePanel 会先占满左侧整列，把 log 挤到右半。本项目顺序：top toolbar → bottom log → left tree → CentralPanel(grid)。StatusBar 不再放外层 `TopBottomPanel`，而是 GridSection 子区域（@04.1.3），通过 `child_ui` 在 CentralPanel 内部分割画出，这样宽度只跟随 GridSection 不跨 TreeSection。
...
无 GUI 环境（CI/CD、jenkins、容器）下使用，与 GUI 共享同一份 `tbl-core`，行为完全一致。`crates/app-cli`（产物名 `tbl-cli`）已实现，是**核心层**功能（@02）的薄入口，不引入任何 UI 渲染依赖。
