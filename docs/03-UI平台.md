...
无 GUI 环境（CI/CD、jenkins、容器）下使用，与 GUI 共享同一份 `tbl-core`，行为完全一致。`crates/app-cli`（产物名 `tbl-cli`）已实现，覆盖 export / validate / generate-test 三条命令。
...
| 子命令 | 说明 | 关键参数 |
|--------|------|----------|
| `export` | 导出数据/代码。无标志=全量；指定标志=按需 | `--json --xml --java --go --lua` |
| `validate` | 全项目验证；有错时 exit 1 | — |
| `generate-test` | 生成内置或外部 schema 的测试数据 | `--empty / --schema <path> / --rows N / --seed N / --format json\|xml / --lang java\|go\|none` |

`-w` 默认当前目录；`-s key=value` 覆盖 `tbl-tool.toml` 任意配置项（@07.4），可重复指定。覆盖键名与 toml 路径一一对应（如 `export.server.java.package`、`export.client.lua.output`）。
