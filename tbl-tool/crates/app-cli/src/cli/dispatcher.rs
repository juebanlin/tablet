...
//! → 翻译 exit code。
//!
//! [`run_with_args`] 也被 `tbl-slint.exe` 在"有 CLI 子命令"时直接调，
//! 等价于跑了一次 `tbl-cli.exe` 子进程。
//!
...
use tbl_core::project::{load_project, load_specific_project};
...
fn run(cli: Cli) -> Result<i32> {
    ...
    match cli.command {
        Command::Export { json, xml, java, go, lua, gdscript } => {
            let summary: ExportSummary = run_export(
                &mut engine,
                ExportFormats { json, xml, java, go, lua, gdscript },
            );
