#!/bin/bash
# tests/10_performance/test_excel_perf.sh — Excel 导出性能阶梯测试
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/../helpers/common.sh"
begin_test "perf/excel"
...
for ROWS in "${TIERS[@]}"; do
    ...
    START=$(date +%s%N)
    run_cli excel export --group hero -o "$WORK_DIR/perf.xlsx"
    END=$(date +%s%N)
