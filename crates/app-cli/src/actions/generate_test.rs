...
pub fn run_generate_test(
    ...
    let core_opts = tablet_core::test_util::TestGenOptions {
    ...
    if let Some(schema_path) = &opts.schema {
        ...
        let parsed = tablet_core::tblschema::parse_tblschema(&content)
            .map_err(|e| anyhow::anyhow!("解析 schema 失败: {}", e))?;
        tablet_core::test_util::generate_from_schema(&config_dir, &parsed, &core_opts);

        match opts.lang.as_str() {
            "java" => tablet_core::test_util::generate_test_main_from_schema(workdir, &parsed, &java_pkg, &opts.format),
            "go" => tablet_core::test_util::generate_test_main_go_from_schema(workdir, &parsed, &go_pkg, &go_code_output, &opts.format),
            "none" => {}
            other => anyhow::bail!("未知 --lang: {}", other),
        }
    } else {
        tablet_core::test_util::generate_test_config(&config_dir, &core_opts);

        match opts.lang.as_str() {
            "java" => tablet_core::test_util::generate_test_main(workdir, &core_opts, &java_pkg, &opts.format),
            "go" => tablet_core::test_util::generate_test_main_go(workdir, &core_opts, &go_pkg, &go_code_output, &opts.format),
            "none" => {}
            other => anyhow::bail!("未知 --lang: {}", other),
        }
    }
